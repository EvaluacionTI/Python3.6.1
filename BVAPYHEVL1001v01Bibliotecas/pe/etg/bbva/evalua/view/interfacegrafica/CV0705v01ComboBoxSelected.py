...
class CVComboBox(oFrameMain.Frame):
    def __init__(self, powindow_root):
        ...
        self.combo.bind("<<ComboboxSelected>>", self.fnc_selection_changed)

    def fnc_selection_changed(self, event):

        # Establecer el contenido en la lista
        #self.combo.set("PYTHON")
        # Obtener todas las opciones de la lista
        #values = self.combo['values']
        print("Elemento seleccionado : ", self.combo.get())
        print("Indice seleccionado : ", self.combo.current())
        #print("Adicionar elemento : ", self.comboset("PYTHON"))
