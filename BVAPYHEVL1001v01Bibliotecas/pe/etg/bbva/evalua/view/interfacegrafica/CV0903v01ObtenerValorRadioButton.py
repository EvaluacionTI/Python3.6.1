...
oRadioCanalAPX = oFormatRadioButton(oWindowRoot, text="APX", value=1, variable=selected)
oRadioCanalAPX.grid(column=1, row=1)

oRadioCanalHost = oFormatRadioButton(oWindowRoot, text="HOST", value=2, variable=selected)
oRadioCanalHost.grid(column=1, row=2)

oButtonSeleccion = oFormat.Button(oWindowRoot, text="Pulsar ", command=clicked)
oButtonSeleccion.grid(column=1, row=3)
